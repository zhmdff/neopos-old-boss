...
const WorkshopAddModal = ({ isOpen, onClose, onRefresh }) => {
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState({
    nameAz: '',
    isPrinting: true,
    printerType: 'None',   // Backend üçün vacibdir
    printerValue: 'Default' // Backend üçün vacibdir
  });

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.nameAz.trim()) return;

    setLoading(true);
    try {
      const user = JSON.parse(localStorage.getItem('user'));
      
      // Backend-in gözlədiyi WorkshopPostDto formatı
      const payload = {
      ...
      onClose();
      // Formanı sıfırla
      setFormData({ nameAz: '', isPrinting: true, printerType: 'None', printerValue: 'Default' });
    } catch (err) {
      console.error("Yaratma xətası:", err.response?.data);
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/40 backdrop-blur-md animate-fadeIn">
      <div className="bg-white w-full max-w-lg rounded-[3rem] shadow-2xl overflow-hidden border border-gray-100">
        <div className="p-8 border-b border-gray-50 flex items-center justify-between bg-gray-50/50">
          <div>
            <h2 className="text-2xl font-black text-gray-900 uppercase tracking-tight">Yeni Emalatxana</h2>
            <p className="text-[10px] font-bold text-[#0ea5e9] uppercase mt-2 italic tracking-widest">İstehsalat nöqtəsi əlavə et</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          <div>
            ...
            <input 
              type="text" required placeholder="Məsələn: Mətbəx, Bar..."
          ...
          >
            {loading ? "Yadda saxlanılır..." : "Təsdiqlə"}
          </button>
        </form>
      </div>
    </div>
...
export default WorkshopAddModal;