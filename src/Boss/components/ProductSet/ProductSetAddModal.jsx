...
const ProductSetAddModal = ({ isOpen, onClose, onRefresh, lunchMode = false }) => {
  ...
  const fetchData = useCallback(async () => {
    if (!companyId) return;
    try {
      // Texnoloji kart üçün yalnız bu şirkətin məhsulları gəlməlidir
      const prodRes = await api.get(`/Products?companyId=${companyId}&take=1000`);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setFormData({
        ...
        description: '',
        setItems: [],
        choiceGroups: lunchMode ? [newChoiceGroup()] : [],
      });
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.productId) return alert("Əsas məhsulu seçin!");
    ...
    if (lunchMode) {
      if (choicePayload.length === 0) {
        return alert('Business lunch üçün ən azı bir seçim qrupu (variantlarla) əlavə edin.');
      }
    } else if (setLines.length === 0) {
      return alert('Ən azı bir tərkib sətri əlavə edin.');
    }
    ...
    setLoading(true);
    try {
      // 3. Backend-dəki CreateSetAsync metoduna uyğun obyekt
      const payload = {
        productId: formData.productId,
        description: formData.description,
        companyId: companyId, // Mütləqdir
        setItems: lunchMode
          ? []
          : setLines.map((item) => ({
  ...
  const modalBody = (
    <div className="fixed inset-0 !left-0 !top-0 w-screen h-screen z-[99999] flex items-center justify-center overflow-hidden">
      ...
      <div className="bg-white w-[95%] max-w-4xl rounded-[2.5rem] shadow-2xl flex flex-col max-h-[92vh] border border-gray-100 relative z-10 animate-modalIn overflow-hidden text-black">
        ...
        <div className="px-8 py-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50 rounded-t-[2.5rem] flex-shrink-0">
          <div className="text-left">
            <h2 className="text-xl font-black text-[#0f172a] uppercase italic tracking-tighter">
              {lunchMode ? 'Yeni Business lunch' : 'Yeni Tex/Karta (Set) Yarat'}
            </h2>
            {!lunchMode ? (
              <p className="mt-1 text-[10px] font-bold uppercase italic tracking-widest text-gray-400">
                Məhsulların tərkibini təyin et
              </p>
            ) : null}
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 overflow-y-auto custom-scrollbar flex-1 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2 text-left">
              <label className="text-xs font-black text-gray-400 uppercase ml-2 italic tracking-wider">Əsas Məhsul (Set Adı)</label>
              <select 
                required
                value={formData.productId}
                onChange={(e) => setFormData({...formData, productId: e.target.value})}
                className="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:border-[#0ea5e9] focus:bg-white outline-none font-bold transition-all text-black"
              >
                <option value="">{lunchMode ? 'Lunch məhsulu seçin…' : 'Set seçin…'}</option>
                {allProducts.map(p => <option key={p.id} value={p.id}>{p.nameAz} ({p.salePrice} ₼)</option>)}
              </select>
            </div>
            
            <div className="space-y-2 text-left">
              <label className="text-xs font-black text-gray-400 uppercase ml-2 italic tracking-wider">Təsvir (Qeyd)</label>
              <input 
                type="text"
                placeholder={lunchMode ? 'Lunch haqqında qısa qeyd…' : 'Set haqqında qısa qeyd…'}
          ...
          {!lunchMode && (
            <div className="rounded-[2.5rem] border border-blue-100/50 bg-sky-50/30 p-6">
              <div className="mb-6 flex items-center justify-between">
                <h3 className="text-sm font-black uppercase italic tracking-widest text-[#0ea5e9]">Setin tərkibi</h3>
                <button
                  type="button"
                  onClick={addItem}
                ...
                >
                  <FiPlus size={14} /> Məhsul əlavə et
                </button>
              </div>

              <div className="space-y-3">
                {formData.setItems.map((item, index) => (
                  <div key={index} className="animate-itemIn flex items-center gap-4 rounded-2xl border border-gray-100 bg-white p-3 shadow-sm">
                ...
                {formData.setItems.length === 0 && (
                  <div className="rounded-[2.5rem] border-2 border-dashed border-blue-100 py-12 text-center text-sm font-bold italic text-blue-300">
                    Tərkib boşdur. Məhsul əlavə edin.
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="p-8 border-t border-gray-100 bg-gray-50/50 rounded-b-[2.5rem] flex gap-4 flex-shrink-0">
          ...
          <button 
            ...
            disabled={loading}
            className="flex-[2] py-4 bg-[#0ea5e9] text-white rounded-2xl font-black uppercase text-[10px] shadow-xl shadow-blue-200 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50"
          >
            {loading ? "Yaradılır..." : "Təsdiqlə və Yarat"}
          </button>
        </div>
      </div>
      ...
      <style>{`
        ...
        .animate-itemIn { animation: itemIn 0.2s ease-out forwards; }
...
export default ProductSetAddModal;