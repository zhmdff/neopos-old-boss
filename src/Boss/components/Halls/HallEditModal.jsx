...
import { FiX, FiLayers, FiPercent } from 'react-icons/fi';
import api from '../../../api/axios';
import BossModalRoot from '../BossModalRoot';

const HallEditModal = ({ isOpen, onClose, onRefresh, hallData }) => {
  ...
  useEffect(() => {
    if (hallData && isOpen) {
      setFormData({
        id: hallData.id,
        nameAz: hallData.nameAz || '',
        servicePercentage: hallData.servicePercentage || 0,
        isGuestCountEnabled:
          hallData.isGuestCountEnabled ??
          hallData.IsGuestCountEnabled ??
          true,
        isTableHourActive:
          hallData.isTableHourActive ?? hallData.IsTableHourActive ?? false,
      });
    }
  }, [hallData, isOpen]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.put('/Halls', payload);
      onRefresh();
      onClose();
    } catch (err) {
      alert(err.response?.data?.message || 'Zal yenilənərkən xəta baş verdi!');
    } finally {
      setLoading(false);
    }
  };
    ...
    <BossModalRoot isOpen={isOpen} onBackdropClose={onClose} maxWidth="max-w-md">
      <div className="flex max-h-[min(90dvh,680px)] flex-col overflow-hidden rounded-[2rem] border border-gray-100 bg-white shadow-2xl sm:rounded-[2.5rem]">
        <div className="flex shrink-0 items-center justify-between border-b border-gray-50 bg-gray-50/50 px-6 py-5">
          <h2 className="text-xl font-black uppercase tracking-tight text-gray-900 sm:text-2xl">Zalı yenilə</h2>
          <button
            type="button"
            onClick={onClose}
          ...
          <div className="flex items-center justify-between gap-4 rounded-2xl border border-amber-100 bg-amber-50/50 px-4 py-3.5">
            <div className="min-w-0">
              ...
              <div className="text-[11px] font-semibold text-amber-700/80">
                Masalar üçün saat:dəqiqə limiti (terminal timer)
              </div>
            </div>
            <button
              type="button"
