...
export default function ReportsByTablesPage() {
  const user = useMemo(() => {
    try {
  ...
  /** Cari növbənin açılışından bu günə qədər tarix sətirləri (by-table API üçün). */
  ...
  const fetchReport = useCallback(async () => {
    ...
    try {
      if (activeRange === 'shift') {
        ...
        const res = await api.get(`/Reports/shift/${shiftId}/by-table`, {
      ...
      const res = await api.get('/Reports/by-table', {
      ...
      <div className="mb-10 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6">
        <div>
          <h1 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tighter italic uppercase leading-none">
            Masalara görə hesabat
          </h1>
          <p className="text-gray-400 font-bold uppercase text-[10px] tracking-[0.2em] mt-2">
            {activeRange === 'shift'
              ? 'Cari növbə · hər masa üzrə satış və sifariş sayı'
              : 'Hər masa üzrə satış və sifariş sayı'}
          </p>
        </div>
          ...
          <div className="bg-white rounded-[40px] border border-gray-100 shadow-sm overflow-hidden">
            <div className="p-6 md:p-8 border-b border-gray-50 flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">Masalar</p>
              ...
              <table className="w-full min-w-[320px] text-left border-collapse">
                <thead>
                  <tr className="border-b border-gray-100 bg-slate-50/80">
                    <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-gray-400">Zal</th>
                    <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-gray-400">Masa</th>
                ...
                <tbody>
                  {items.map((x, idx) => (
                    <tr
                      key={x.tableId || x.TableId || idx}
                      className={`border-b border-gray-50 ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/40'}`}
                    >
                      <td className="px-6 py-4 text-sm font-bold text-slate-600 truncate max-w-[140px]">
                        {x.hallName || x.HallName || '—'}
                      </td>
                      <td className="px-6 py-4 text-sm font-black text-slate-900 uppercase truncate max-w-[180px]">
                        {x.tableName || x.TableName || 'Masa'}
                      </td>
